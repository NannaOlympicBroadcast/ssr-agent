...
def load_plugins(settings: Settings) -> list[ContextItem]:
    ...
    items: list[ContextItem] = []
    for base in (settings.plugins_dir, settings.project_state_dir / "plugins"):
        if not base.exists():
            continue
        for manifest in base.glob("*/.claude-plugin/plugin.json"):
            text = _read_text(manifest)
            if text.strip():
                items.append(ContextItem(ContextCategory.SKILLS, f"plugin {manifest.parent.parent.name}", text, str(manifest), {"kind": "plugin"}))
