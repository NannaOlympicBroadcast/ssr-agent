...
def run_hooks(settings: Settings, event: str, payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for h in load_hooks(settings).get(event, []):
        ...
        if not cmd:
            continue
        try:
            proc = subprocess.run(cmd, input=json.dumps(payload), text=True, shell=True, cwd=settings.project_dir, capture_output=True, timeout=int(h.get("timeout", 30)))
            results.append({"event": event, "command": cmd, "exit_code": proc.returncode, "stdout": proc.stdout, "stderr": proc.stderr})
